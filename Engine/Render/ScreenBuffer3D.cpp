#include "ScreenBuffer3D.h"
#include <cassert>
#include <iostream>

ScreenBuffer3D::ScreenBuffer3D(const RenderPosition& screenSize)
		: size(screenSize)
{
	// 콘솔 버퍼 생성
...
ScreenBuffer3D::~ScreenBuffer3D()
{
    if (buffer) CloseHandle(buffer);   // Window에서 넘겨받은 핸들 반환
}

void ScreenBuffer3D::Clear() const
{
...
void ScreenBuffer3D::Draw(const CHAR_INFO* const charInfo) const
{
